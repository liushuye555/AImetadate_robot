...
def config_schema(data: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        ...
        field('echo.enabled', 'bool', echo.get('enabled', False), 'Echo/repeat enabled', '复读机开关', section='echo'),
        field('echo.min_repeat', 'number', echo.get('min_repeat', 3), 'Repeat threshold', '复读触发人数', section='echo', min=2, max=50),
        field('echo.window_seconds', 'number', echo.get('window_seconds', 60), 'Repeat window (s)', '复读时间窗（秒）', section='echo', min=10, max=3600),
        ...
        field('keepalive.enabled', 'bool', keepalive.get('enabled', False), 'Enable scheduled group keepalive', '启用定时群保活', section='features'),
        field('keepalive.interval_minutes', 'number', keepalive.get('interval_minutes', 0), 'Keepalive interval (minutes)', '群保活间隔（分钟）', section='features', min=0, max=10080),
        field('keepalive.groups', 'list', keepalive.get('groups', []), 'Keepalive groups', '保活群列表', section='features'),
        field('keepalive.message', 'text', keepalive.get('message', ''), 'Keepalive message', '保活消息', section='features'),
    ]
