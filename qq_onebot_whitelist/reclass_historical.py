...
- PNG 顺带扫 alpha 隐写：容器本身不是来源证据，载荷文本的强证据说了算
  （普通参数块 → A1111，NovelAI 专有键 → NovelAI，验证不出 → 保留未知）；
- 同一 sha 只解析一次，结果复用到所有引用该内容的记录。

用法：
    python -m qq_onebot_whitelist.reclass_historical data/bot.db            # dry-run
    python -m qq_onebot_whitelist.reclass_historical data/bot.db --execute  # 写库
"""
...
def _verdict_from_meta(meta, meta_keys: list[str]) -> tuple[int, str]:
    ...
    if meta.stego_state == 'verified':
        # 隐写容器本身不是来源证据：载荷文本的强证据说了算，
        # 验证不出工具时保留未知（ai_source 置空）。
...
def reclass_historical(db_path: str | Path, *, dry_run: bool = True) -> dict[str, object]:
    ...
    try:
        ...
        for row in rows:
            ...
            if old_has == has_ai and old_src == source:
                continue
            planned += 1
            transition = f'{old_src or "(none)"} -> {source or "(none)"}'
            changed[transition] += 1
            if len(samples) < SAMPLE_LIMIT:
                samples.append(f'id={row["id"]} {transition}')
            if dry_run:
                continue
            conn.execute(
        ...
        if not dry_run:
            conn.commit()
        result = {'records': len(rows), 'unique_contents': len(verdicts),
                  'planned_changes': planned, 'dry_run': 1 if dry_run else 0, **stats}
...
if __name__ == '__main__':
    ...
    for key, value in reclass_historical(db, dry_run=not execute).items():
        print(f'{key}: {value}')
