...
if(collapse){{const groups={{}};cells.forEach(a=>{{const m=(a.getAttribute('href')||'').match(/_pk([0-9a-f]{{8}})/);if(m){{const k=m[1];(groups[k]=groups[k]||[]).push(a);}}}});let folded=0;Object.values(groups).forEach(g=>{{if(g.length>1&&g.length<=30){{folded++;const b=document.createElement('span');b.style.cssText='position:absolute;top:6px;left:6px;background:#000c;color:#ffd98a;font-size:12px;padding:2px 8px;border-radius:999px;cursor:pointer;z-index:2';g[0].style.position='relative';g[0].appendChild(b);const paint=()=>{{b.textContent=b.dataset.open==='1'?('同批 '+g.length+' 张 · 点击收起'):('同批 '+g.length+' 张 · 点击展开');}};const render=()=>{{g.forEach((a,i)=>{{a.style.display=(i>0&&b.dataset.open!=='1')?'none':'';}});paint();}};b.addEventListener('click',e=>{{e.preventDefault();e.stopPropagation();if(b.dataset.open!=='1'){{let ref=g[0];g.forEach((a,i)=>{{if(i>0){{a.style.display='';ref.parentNode.insertBefore(a,ref.nextSibling);ref=a;}}}});b.dataset.open='1';}}else{{g.forEach((a,i)=>{{if(i>0){{a.style.display='none';}}}});b.dataset.open='0';}}render();}});render();}}}});const ci=document.getElementById('collapseInfo');if(ci&&folded){{ci.textContent='已折叠 '+folded+' 组同批（≤30 张），点角标展开/收起';}}}}
